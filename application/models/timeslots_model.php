...
class Timeslots_model extends CI_Model {
     ...
     */
    public function book_room($room, $startdate, $enddate, $status, $creator,$note) {
//        $room = $this->input->post('room');
//        $startdate = $this->input->post('startdate');
//        $enddate = $this->input->post('enddate');
//        $status = $this->input->post('status');
//        $creator = $this->input->post('creator');
//        $note = $this->input->post('note');// add note call fuction from set_timeslots
//   add accept automatically
       
        $this->set_timeslots($room, $startdate, $enddate, $status, $creator,$note);
        
        return $this->db->insert_id();
    }
    // select room where id to get startdate and enddate to validate time
    public function get_time($room){
        
        $query= $this->db->get_where('timeslots', array('room' => $room));
        $result = $query->result();
        return $result;
    }
    ...
    /** get status 3 and timedate to do user available*/
    public function get_status_date(){
       $query = $this->db->get_where('timeslots', array('status' => 3));
       return $query->result();
    }
    /**
    ...
    // get construct room
    public function get_construct($id){
        $this->db->where('id',$id);
        $query=$this->db->get('timeslots');
        return $query->result();
    }
    // this function delete contruction
    public  function delete_construct($id){
        $this->db->where('id',$id);
        $query=$this->db->delete('timeslots');
        //return $query->result();
    }
