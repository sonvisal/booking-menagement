...
class Users_model extends CI_Model {
    ...
    // function set besy user

    public function set_busy($user) {
    $this->db->where('id', $user);
    return $this->db->update('users', array('free' => FALSE));
        }

    // function set user free
        public function set_free($user) {
    $this->db->where('id', $user);
    return $this->db->update('users', array('free' => TRUE));
        }

    public function update_availability() {
    $data = array('free' => FALSE);
    $this->db->where('timeslots.status', 3);  //Only accepted status
    $this->db->where('NOW() >= timeslots.startdate');
    $this->db->where('NOW() <= timeslots.enddate');
    return $this->db->update('users join timeslots on users.id = timeslots.creator', $data);
    }

    public function update_available() {
    $data = array('free' => TRUE);
    $this->db->where('timeslots.status', 3);  //Only accepted status
    //$this->db->where('NOW() <= timeslots.startdate');
    $this->db->where('NOW() >= timeslots.enddate');
    return $this->db->update('users join timeslots on users.id = timeslots.creator', $data);
