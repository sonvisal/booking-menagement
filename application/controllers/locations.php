...
class Locations extends CI_Controller {
    ...
    public function create() {
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            $this->locations_model->insert_map();
    ...
    public function edit($id) {
        ...
        $this->form_validation->set_rules('lattitude', lang('locations_edit_field_lattitude'), 'required');
        $this->form_validation->set_rules('longitude', lang('locations_edit_field_longitude'), 'required');
        if ($this->form_validation->run() === FALSE) {
            $this->load->model('locations_model');
            $data['map']=$this->locations_model->get_map($id);
            $data['location'] = $this->locations_model->get_locations($id);
            $this->load->view('templates/header', $data);
            $this->load->view('menu/index', $data);
            $this->load->view('locations/edit', $data);
            $this->load->view('templates/footer');// Fix error 1 add header and style
        } else {
            $this->locations_model->update_locations();
            $this->locations_model->update_map();
